...
func PropagateCommand(command string, args []protocol.Value) {
	...
	replicasMu.RLock()
	snapshot := make(map[string]net.Conn, len(server.StoreState.Replicas))
	for id, c := range server.StoreState.Replicas {
		snapshot[id] = c
	}
	replicasMu.RUnlock()
...
func ReplicasGet(connID string) (net.Conn, bool) {
	replicasMu.RLock()
	c, ok := server.StoreState.Replicas[connID]
	replicasMu.RUnlock()
	return c, ok
}
...
func ReplicasSet(connID string, conn net.Conn) {
	replicasMu.Lock()
	server.StoreState.Replicas[connID] = conn
	replicasMu.Unlock()
}

func ReplicasDelete(connID string) {
	replicasMu.Lock()
	delete(server.StoreState.Replicas, connID)
	replicasMu.Unlock()
}
...
func SendReplconfGetack() {
	...
	replicasMu.RLock()
	replicas := make(map[string]net.Conn, len(server.StoreState.Replicas))
	for id, c := range server.StoreState.Replicas {
		replicas[id] = c
	}
	replicasMu.RUnlock()
